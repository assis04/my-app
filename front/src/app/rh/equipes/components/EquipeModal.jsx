        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-8 space-y-8 bg-white custom-scrollbar">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2 focus-within:scale-[1.01] transition-transform">
              <label className="text-sm font-bold text-slate-400 ml-1">Identificação da Equipe *</label>
              <input
                required type="text" placeholder="Ex: Comercial Sul"
                className="w-full bg-slate-50 text-slate-900 p-3 rounded-2xl border border-slate-200 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all font-black placeholder:text-slate-300 text-base"
                value={formData.nome}
                onChange={e => setFormData(p => ({ ...p, nome: e.target.value }))}
              />
            </div>
            <div className="space-y-2 focus-within:scale-[1.01] transition-transform">
              <label className="text-sm font-bold text-slate-400 ml-1">Objetivo / Descrição</label>
              <input
                type="text" placeholder="Foco em prospecção..."
                className="w-full bg-slate-50 text-slate-900 p-4 rounded-2xl border border-slate-200 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all font-bold placeholder:text-slate-300 text-base"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-400 ml-1">Liderança</label>
              <PremiumSelect 
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-400 ml-1">Unidade</label>
              <PremiumSelect 
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-400 ml-1">Operante</label>
              <PremiumSelect 
                options={[
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <label className="text-base font-bold text-slate-800">Contingente da Equipe</label>
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-slate-50 text-base text-slate-900 pl-12 pr-6 py-3 rounded-2xl border border-slate-200 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all font-bold placeholder:text-slate-300"
                value={searchMembro}
            ...
            <div className="max-h-56 overflow-y-auto custom-scrollbar rounded-2xl border border-slate-200 bg-slate-50/50 p-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {loadingDeps ? (
                ...
                <div className="col-span-full text-center py-10">
                   <p className="text-slate-400 font-medium text-base">Nenhum colaborador localizado.</p>
                </div>
              ) : filteredUsers.map(u => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center text-xs text-slate-600 font-black shrink-0 transition-transform group-hover:scale-110">
                      {u.nome.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm text-slate-800 font-black truncate">{u.nome}</p>
