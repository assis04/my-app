  ...
  const RenderedQueue = useMemo(() => {
    return queue.map((agent, index) => (
      <div key={agent.id} className="flex flex-col sm:flex-row sm:items-center py-0.5 border-b border-slate-50 last:border-0 gap-2 sm:gap-0 transition-all hover:bg-slate-50/80 px-3 rounded-xl group">
        <span className="w-8 text-slate-300 font-black text-base italic tracking-tighter group-hover:text-sky-400 transition-colors">
          {String(index + 1).padStart(2, '0')}
        </span>
        
        <div className="flex items-center gap-2 flex-1">
          <span className="text-slate-400 whitespace-nowrap text-xs font-black uppercase tracking-tighter italic">Atribuir:</span>
          <form onSubmit={(e) => submitDirectLead(agent.id, agent.nome, e)} className="w-full sm:w-auto flex items-center relative">
            ...
            {agent.isAvailable && index === firstAvailableIndex && !phoneInputs[agent.id] && (
              <div className="absolute right-2 top-1/2 -translate-y-1/2 bg-sky-100 text-sky-600 px-1 py-0.5 rounded text-xs font-black uppercase pointer-events-none">OK</div>
            )}
        ...
        <div className="flex-1 text-slate-900 flex items-center justify-between pl-4">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <span className="font-black text-base text-slate-900 group-hover:text-sky-700 transition-colors leading-tight uppercase tracking-tight">{agent.nome}</span>
              <span className="text-xs font-bold text-slate-400 leading-none mt-0 italic uppercase tracking-tighter">Vendedor</span>
            </div>
            {!agent.isAvailable ? (
              <span className="text-xs font-black text-rose-600 border border-rose-100 bg-rose-50 px-1.5 py-0.5 rounded-full shadow-xs uppercase">Ausente</span>
            ) : agent.isAvailable && index === firstAvailableIndex ? (
              <span className="text-xs font-black text-emerald-600 border border-emerald-100 bg-emerald-50 px-1.5 py-0.5 rounded-full shadow-xs uppercase">Na Vez</span>
            ) : (
              <span className="text-xs font-black text-sky-400 border border-sky-100 bg-sky-50 px-1.5 py-0.5 rounded-full shadow-xs uppercase tracking-tighter">Livre</span>
            )}
          </div>
          ...
          {(user?.id === agent.id || canToggleOthers) && (
             <button 
               ...
               className="text-xs font-black uppercase tracking-tighter text-sky-600 hover:text-white hover:bg-sky-600 px-2.5 py-1 rounded-xl border border-sky-100 bg-sky-50 transition-all shadow-xs active:scale-95 whitespace-nowrap"
             >
        ...
        <div className="mb-4 max-w-[1600px] mx-auto">
          ...
          <div className="w-full relative glass-card border border-white/60 rounded-3xl p-1 shadow-floating overflow-hidden transition-all bg-white/40 backdrop-blur-xl">
            ...
            {queueLoading && queue.length === 0 && (
               <div className="text-slate-400 py-3 opacity-70 flex items-center justify-center font-black text-xs uppercase animate-pulse">
                 Sincronizando...
               </div>
            )}
            {queue.length === 0 && !queueLoading && (
               <div className="text-slate-400 py-4 text-center font-black uppercase text-xs italic">Vazio.</div>
            )}
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-4 gap-2">
              ...
              <div className="relative flex items-center gap-2">
                <div className={`flex items-center gap-2 bg-white border border-slate-200 rounded-2xl px-1 h-9 shadow-xs transition-all w-full ${filterData ? 'ring-2 ring-sky-500/20 border-sky-200' : ''}`}>
                  ...
                  <input 
                    ...
                    className="bg-transparent text-sm text-slate-900 pr-2 py-0 outline-none font-bold cursor-pointer uppercase tracking-tighter"
                  />
                  {filterData && (
                    <button 
            ...
            <div className="flex items-center gap-3 w-full xl:w-auto justify-end">
              <div className="flex-col items-end mr-1 hidden xl:flex leading-none">
                <span className="text-xs text-slate-300 font-black uppercase tracking-widest italic leading-none">Unidade</span>
                <span className="text-xs font-black text-sky-500 uppercase tracking-tighter leading-none mt-0.5">
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-linear-to-r from-sky-500 to-sky-600 text-white px-4 py-2 rounded-2xl hover:shadow-sky-500/40 hover:shadow-2xl font-black shadow-xl shadow-sky-900/10 transition-all text-xs active:scale-95 whitespace-nowrap uppercase tracking-widest"
              >
                Novo 
                <Plus size={14} />
              </button>
                  ...
                  {filteredHistory.map((lead) => (
                    ...
                    >
                      ...
                      <td className="py-1.5 px-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-black border shadow-xs uppercase tracking-tighter ${
                      ...
                      <td className="py-1.5 px-3">
                        <span className="text-xs font-black text-sky-500 bg-sky-50 px-2 py-0.5 rounded-lg border border-sky-100 uppercase tracking-tighter">{lead.etapa || 'Novo'}</span>
                      </td>
                      <td className="py-1.5 px-3">
                        <div className="flex flex-col leading-tight">
                          <span className="text-slate-900 text-sm font-black group-hover:text-sky-700 transition-colors uppercase tracking-tight truncate max-w-[150px]">{lead.nome || '—'}</span>
                          <span className="text-xs text-slate-400 font-bold uppercase tracking-tighter">{lead.celular || lead.telefone || '—'}</span>
                        </div>
                      </td>
                      <td className="py-1.5 px-3">
                        <span className="text-slate-400 text-xs font-black bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100 uppercase tracking-tighter">{(lead.vendedor || lead.user)?.nome || '???'}</span>
                      </td>
                      <td className="py-1.5 px-3">
                        <div className="flex flex-col leading-none">
                          <span className="text-xs font-black text-sky-400 uppercase tracking-tighter italic">{lead.canal || 'N/D'}</span>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-tighter">{lead.origem || '—'}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-1.5 px-4 text-slate-300 text-xs font-black text-right uppercase italic tracking-tighter">{timeAgo(lead.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
