    ...
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="glass-card rounded-2xl p-4 mb-6 relative border border-white/60 shadow-floating bg-white/40 backdrop-blur-xl">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h2 className="text-base font-black text-slate-900 flex items-center gap-2 uppercase tracking-tight italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          <div className="relative group">
            ...
            <input
              ...
              className="premium-input py-2 pl-10 text-sm shadow-xs font-bold"
            />
          </div>
      ...
      <div className="flex justify-between items-center mb-4 px-2">
        <div className="text-zinc-500 text-xs uppercase font-black bg-slate-100/50 px-3 py-1 rounded-full border border-slate-200 tracking-tighter shadow-xs italic">
      ...
      <div className="w-full bg-white border border-slate-100 rounded-2xl shadow-floating overflow-hidden mb-6">
        <div className="overflow-x-auto w-full scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
          <div className="min-w-[1100px] w-full">
            <div className="grid grid-cols-[120px_3fr_2fr_2fr_2fr_1.2fr_1.2fr] gap-3 py-3 bg-slate-50/50 border-b border-slate-100 text-slate-500 font-black text-xs uppercase px-4 italic tracking-tighter">
              <div>Número</div>
              <div>Lead</div>
              <div>Contato</div>
              <div>Responsável</div>
            ...
            {loading && (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-slate-400 font-black text-xs uppercase animate-pulse">Sincronizando base...</p>
              </div>
            )}
            ...
            {!loading && orcamentos.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-slate-400 font-black text-xs uppercase">Nenhum orçamento encontrado.</p>
              ...
              orcamentos.map((orc) => {
                ...
                return (
                  <div
                    ...
                    className="grid grid-cols-[120px_3fr_2fr_2fr_2fr_1.2fr_1.2fr] gap-3 py-2.5 border-b border-slate-50 hover:bg-sky-50 transition-all items-center text-sm px-4 group cursor-pointer"
                  >
                    <div className="truncate font-black text-sky-600 tracking-tighter text-xs uppercase" title={orc.numero}>
                      {orc.numero}
                    </div>
                    <div className="truncate font-black text-slate-900 group-hover:text-sky-700 uppercase tracking-tighter" title={lead.nome}>
                      {lead.nome || '—'}
                    </div>
                    <div className="truncate text-slate-500 font-bold tracking-tighter uppercase">
                      {lead.celular || '—'}
                    </div>
                    <div className="truncate" title={lead.vendedor?.nome}>
                      <span className="bg-slate-50 px-2 py-0.5 rounded-xl text-xs font-black text-slate-400 border border-slate-100 group-hover:bg-white group-hover:text-slate-600 group-hover:border-slate-200 transition-all uppercase tracking-tighter">
                        {lead.vendedor?.nome || '—'}
                      </span>
                    </div>
                    <div className="truncate text-slate-600 font-bold uppercase tracking-tighter" title={lead.filial?.nome}>
                      {lead.filial?.nome || '—'}
                    </div>
                    <div className="truncate text-slate-400 font-bold text-xs uppercase tracking-tighter">
